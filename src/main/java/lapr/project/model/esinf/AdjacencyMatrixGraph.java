...
public class  AdjacencyMatrixGraph<V, E> extends CommonGraph<V, E> {
    ...
    public Collection<Edge<V, E>> outgoingEdges(V vert) {
        ...
        for (int i = 0; i < numVerts; i++)
            if (edgeMatrix[vKey][i] != null)
                outgoingEdges.add(edgeMatrix[vKey][i]);
        return outgoingEdges;
    }
    ...
    public Collection<Edge<V, E>> incomingEdges(V vert) {
        ...
        for (int i = 0; i < numVerts; i++)
            if (edgeMatrix[i][vertKey] != null)
                incomingEdges.add(edgeMatrix[i][vertKey]);
        return incomingEdges;
    }
    ...
    public Graph<V, E> clone() {
        AdjacencyMatrixGraph g = new AdjacencyMatrixGraph(this.isDirected, this.edgeMatrix.length);
